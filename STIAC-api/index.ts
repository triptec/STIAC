...
async function listCreate(payload, callback) {
  const socket = this;
  const userId = socket.userId;
  const list = await createList(payload.list, userId);
  users[userId].lists[list.id] = list;
  socket.emit(c.events.LISTS_ADD, list);
  callback(list);
}
...
async function tickerAdd(ticker_: Ticker, listId_: any, callback) {
  ...
  socket.emit(c.events.LISTS_TICKERS_ADD, {
    listId: listId_,
    isin: ticker.isin,
  });
  callback({ status: "ok" });
